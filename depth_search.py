...
def Move_hanoi(matrix, state_list, coluna_atual, coluna_destino):   # move da coluna X para a coluna Y
    ...
    if(has_moved):                      # se houve um movimento
        for i in range(0, len(state_list)):
            if i>0 and state_list[i][0] == copy:       # confere se esse estado novo possui possibilidades nao exploradas
                return False

        state_list.append([copy, True])       # se possui, o estado atual 'e confirmado
        matrix[:] = copy
        return True;
